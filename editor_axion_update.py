"""
editor_axion_update.py
Editor de atualizacao do Axion Unlocker.
...
_BTN = f"""
    QPushButton {{
        background: rgba(100,40,180,200);
    ...
    QPushButton:hover {{
        background: rgba(130,60,210,255);
        border-color: rgba(192,132,252,200);
    }}
    QPushButton:pressed {{ background: rgba(80,30,150,255); }}
"""
...
_LIST = f"""
    ...
    QScrollBar::handle:vertical {{
        background: rgba(168,85,247,140); border-radius: 4px; min-height: 24px;
    }}
    QScrollBar::handle:vertical:hover {{ background: rgba(168,85,247,200); }}
...
class EditorWindow(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Axion Unlocker — Editor de Atualização")
        ...
        self.version_data: dict = load_json(VERSION_PATH, {
            "version": "", "axion_release": "", "game_version": ""
        })
    ...
    def _titlebar(self):
        ...
        t = QLabel("AXION UNLOCKER — EDITOR DE ATUALIZAÇÃO")
    ...
    def _style_tabs(self):
        for tid, tab in self._tabs.items():
            if tid == self.current_page:
                tab.setStyleSheet(f"""
                    QLabel {{
                        color:{TEXT}; background:rgba(168,85,247,18);
    ...
    def _page_version(self):
        ...
        # version (usado pelo updater para comparar)
        lbl1 = QLabel("VERSÃO DO UPDATER  (campo \"version\" — usado para comparação)")
        lbl1.setFont(QFont("Segoe UI", 9))
        lbl1.setStyleSheet(f"color:{TEXT_MID}; letter-spacing:1px; background:transparent; border:none;")
        vl.addWidget(lbl1)
        self.entry_version = QLineEdit()
        self.entry_version.setText(self.version_data.get("version", ""))
        self.entry_version.setFont(QFont("Segoe UI", 13))
        self.entry_version.setFixedHeight(42)
        self.entry_version.setPlaceholderText("ex: 1.0.5")
        self.entry_version.setStyleSheet(_INPUT)
        self.entry_version.textChanged.connect(self._on_version_changed)
        vl.addWidget(self.entry_version)

        sep = QFrame(); sep.setFixedHeight(1)
        sep.setStyleSheet(f"background:{BORDER}; border:none;")
        vl.addWidget(sep)

        # axion_release (exibido no título do painel)
        lbl2 = QLabel("RELEASE DO AXION  (campo \"axion_release\" — exibido no título)")
        lbl2.setFont(QFont("Segoe UI", 9))
        lbl2.setStyleSheet(f"color:{TEXT_MID}; letter-spacing:1px; background:transparent; border:none;")
        vl.addWidget(lbl2)
        self.entry_axion_release = QLineEdit()
        self.entry_axion_release.setText(self.version_data.get("axion_release", ""))
        self.entry_axion_release.setFont(QFont("Segoe UI", 13))
        self.entry_axion_release.setFixedHeight(42)
        self.entry_axion_release.setPlaceholderText("ex: v1.0.5")
        self.entry_axion_release.setStyleSheet(_INPUT)
        vl.addWidget(self.entry_axion_release)

        sep2 = QFrame(); sep2.setFixedHeight(1)
        sep2.setStyleSheet(f"background:{BORDER}; border:none;")
        vl.addWidget(sep2)

        # game_version (versão do DBD)
        lbl3 = QLabel("VERSÃO DO DEAD BY DAYLIGHT  (campo \"game_version\")")
        lbl3.setFont(QFont("Segoe UI", 9))
        lbl3.setStyleSheet(f"color:{TEXT_MID}; letter-spacing:1px; background:transparent; border:none;")
        vl.addWidget(lbl3)
        self.entry_game_version = QLineEdit()
        self.entry_game_version.setText(self.version_data.get("game_version", ""))
        self.entry_game_version.setFont(QFont("Segoe UI", 13))
        self.entry_game_version.setFixedHeight(42)
        self.entry_game_version.setPlaceholderText("ex: 9.5.2")
        self.entry_game_version.setStyleSheet(_INPUT)
        vl.addWidget(self.entry_game_version)

        note = QLabel(
            "\"version\" é comparado com o version.json local do cliente para decidir se há atualização.\n"
            "\"axion_release\" é exibido no título da janela do painel."
        )
    ...
    def _edit_item(self):
        ...
        if ok and novo.strip():
            changes[row] = f"{PREFIXOS[prefixo_key]} {novo.strip()}" if prefixo_key else novo.strip()
            self._refresh_list()
    ...
    def _on_version_changed(self, text: str):
        """Auto-preenche axion_release com v{version} ao digitar a versão."""
        ver = text.strip()
        if ver:
            release = f"v{ver}" if not ver.startswith("v") else ver
            self.entry_axion_release.setText(release)

    def _save_version(self):
        version       = self.entry_version.text().strip()
        axion_release = self.entry_axion_release.text().strip()
        game_version  = self.entry_game_version.text().strip()
        if not version:
            QMessageBox.warning(self, "Aviso", "Informe o campo \"version\".")
            return
        self.version_data["version"]       = version
        self.version_data["axion_release"] = axion_release
        self.version_data["game_version"]  = game_version
